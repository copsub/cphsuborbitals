<?php
/*
Template Name: Forside template 3 - countdown
*/
?>
...
<?php get_header(); 
  ...
  $image        =  get_field( '_frontpage_template_3_image', $post->ID ); 
  $image_width  =  $image['width'];
  $image_height =  $image['height'];
  $image_url    =  $image['url'];
  $image_url    =  '' !== $image_url ? $image_url : CHILD_THEME_URI . '/img/default.jpg';
  
  $title    =  get_field( '_frontpage_template_3_title', $post->ID );
  $text     =  get_field( '_frontpage_template_3_text',  $post->ID );
  $date     =  esc_attr( strip_tags( get_field( '_frontpage_template_3_date',  $post->ID ) ) );
  
?>

<div class="frontpage template_3" style="background-image:url('<?php echo $image_url; ?>'); background-size:<?php echo $image_width; ?>px <?php echo $image_height; ?>px;" data-image-url="<?php echo $image_url; ?>">
